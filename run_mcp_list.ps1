...
$notifResp = Invoke-WebRequest -Uri $mcpUrl -Method Post -Headers ($headers + @{"mcp-session-id" = $sessionId}) -Body $notifPayload -ContentType "application/json" -SkipHttpErrorCheck
...
$listResp = Invoke-WebRequest -Uri $mcpUrl -Method Post -Headers ($headers + @{"mcp-session-id" = $sessionId}) -Body $listPayload -ContentType "application/json" -SkipHttpErrorCheck
