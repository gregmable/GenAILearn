...
$initRes = Invoke-WebRequest -Uri $mcpUrl -Method Post -Headers $headers -Body ($initPayload | ConvertTo-Json -Depth 10) -SkipHttpErrorCheck -UseBasicParsing
if ($initRes.StatusCode -ne 200) { $initRes.Content; exit }
$mcpSessionId = $initRes.Headers["mcp-session-id"]
$headers["mcp-session-id"] = $mcpSessionId
$notifPayload = @{ jsonrpc = "2.0"; method = "notifications/initialized" }
$notifRes = Invoke-WebRequest -Uri $mcpUrl -Method Post -Headers $headers -Body ($notifPayload | ConvertTo-Json -Depth 10) -SkipHttpErrorCheck -UseBasicParsing
if ($notifRes.StatusCode -ne 200) { $notifRes.Content; exit }
$listPayload = @{ jsonrpc = "2.0"; id = 3; method = "tools/list" }
$listRes = Invoke-WebRequest -Uri $mcpUrl -Method Post -Headers $headers -Body ($listPayload | ConvertTo-Json -Depth 10) -SkipHttpErrorCheck -UseBasicParsing
if ($listRes.StatusCode -ne 200) { $listRes.Content; exit }
